...
##Date: August 19, 2011
##
##Reads daily close prices for a given commodity in a given range of dates. Computes
...
##(also set at 70 and 30, respectively). This file does not read price data from
##Yahoo Finance. It must be paired with a .csv (see RSI-SignalProject Wiki page
##for .csv files). 
...
pxreader = csv.reader(open(ticker + ' Prices.csv', 'rb'))
dateslst = []
priceslst = []
for row in pxreader:
    if row[0] != 'Date':
        dateslst.append(datetime.datetime.strptime(row[0], '%m/%d/%Y').date())
        priceslst.append(float(row[1]))

dates = np.array(dateslst)[9:]
px = np.array(priceslst)
prices = px[9:]
